# Test Plan — AI Evaluator CLI (C# / .NET)
...
## 1. Config Module (`ConfigTests.cs`)
...
## 2. API Client (`ApiClientTests.cs`)
...
| 2.6 | `TestRequestHTTP4xx` | HTTP 400/429 → raises `ApiError` with status code |
| 2.7 | `TestRequestHTTP5xx` | HTTP 500 → raises `ApiError` with status code |
| 2.8 | `TestRequestConnectionRefused` | Cannot connect → raises `ApiError` with status 0 |
| 2.9 | `TestRequestTimeout` | Timeout → raises `ApiError` with status 0 |
...
## 3. Dataset Parsing (`DatasetTests.cs`)
...
## 4. Metrics Parsing (`MetricsTests.cs`)
...
## 5. Per-Metric Thresholds (`ThresholdsTests.cs`)
...
## 6. Output Formatters (`FormatterTests.cs`)
...
## 7. CLI Commands — `login` (`LoginCommandTests.cs`)
...
## 8. CLI Commands — `whoami` (`WhoamiCommandTests.cs`)
...
## 9. CLI Commands — `quick` (`QuickCommandTests.cs`)
...
## 10. CLI Commands — `eval` (`EvalCommandTests.cs`)
...
## 11. CLI Commands — `config` (`ConfigCommandTests.cs`)
...
## 12. CLI Commands — `init` (`InitCommandTests.cs`)
...
## 13. Exit Codes (`ExitCodesTests.cs`)
...
**Test framework:** `xUnit` + `Moq` (for HTTP mocking with `HttpMessageHandler`)
