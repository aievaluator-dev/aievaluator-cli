# Test Plan — AI Evaluator CLI (Python)
...
## 2. API Client (`test_api_client.py`)
...
| 2.5 | `test_api_error_creation` | `APIError` stores status_code, message, detail |
...
## 3. Dataset Parsing (`test_dataset.py`)
...
## 4. Metrics Parsing (`test_metrics.py`)
...
## 5. Per-Metric Thresholds (`test_thresholds.py`)
...
## 6. Output Formatters (`test_formatters.py`)
...
## 7. CLI Commands — `login` (`test_cli_login.py`)
...
## 8. CLI Commands — `whoami` (`test_cli_whoami.py`)
...
## 9. CLI Commands — `quick` (`test_cli_quick.py`)
...
## 10. CLI Commands — `eval` (`test_cli_eval.py`)
...
## 11. CLI Commands — `config` (`test_cli_config.py`)
...
## 12. CLI Commands — `init` (`test_cli_init.py`)
...
## 13. Exit Codes (`test_exit_codes.py`)
