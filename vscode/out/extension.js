...
class EvalHistoryItem {
    ...
    response;
    expected;
    constructor(query, scores, passed, timestamp, evaluationId, response, expected) {
        this.query = query;
        this.scores = scores;
        this.passed = passed;
...
function activate(context) {
    ...
    inlineCustomEvaluators = (context.workspaceState.get('aievaluator.customEvaluators') || []);
    ...
    context.subscriptions.push(vscode.commands.registerCommand('aievaluator.evaluateFile', async () => {
        ...
        const pick = await vscode.window.showQuickPick(datasets.map(d => ({
            label: d.name,
...
// ═══════════════════════════════════════════════════════════════════
async function showCustomEvalPromptPanel(name) {
    const panel = vscode.window.createWebviewPanel('customEvalPrompt', `Custom Evaluator: ${name}`, vscode.ViewColumn.One, { enableScripts: true, retainContextWhenHidden: true });
    ...
    await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'Evaluating...' }, async () => {
        try {
            if (apiKey) {
                ...
                addToHistory(selection, scores, data.results?.[0]?.passed, data.evaluation_id || '', data.results?.[0]?.agent_response, opts.expected);
                ...
                if (evalResult) {
                    ...
                    addToHistory(selection, scores, evalResult.passed, data.evaluation_id || '', evalResult.agent_response, opts.expected);
                    sidebarProvider?.refresh(history);
                    vscode.window.showInformationMessage(`AI Evaluator: ${scoreList} ${passIcon}`);
                }
            }
            ...
            if (msg.toLowerCase().includes('daily limit') || msg.toLowerCase().includes('429')) {
                vscode.window.showWarningMessage(`AI Evaluator: ${msg}`, 'Get API Key').then(action => {
                    if (action === 'Get API Key')
                        vscode.env.openExternal(vscode.Uri.parse('https://www.aievaluator.dev/login'));
                });
            }
            else {
...
async function evaluateDataset(document) {
    ...
    try {
        ...
        vscode.window.showErrorMessage(`AI Evaluator: Could not parse ${document.fileName.split('/').pop()}. Make sure it's valid JSON (array of {input, expected_output?}) or JSONL.`, 'Init example dataset').then(action => {
            if (action === 'Init example dataset')
                vscode.commands.executeCommand('aievaluator.init');
        });
        return;
    }
...
// ═══════════════════════════════════════════════════════════════════
function showResultsPanel(fileName, data) {
    const results = (data.results || []);
    const overallScore = data.overall_score || 0;
    const allPassed = results.every((r) => r.passed);
    // Add to history
    for (const r of results) {
        addToHistory(r.query, (r.scores || {}), r.passed, data.evaluation_id || '', r.agent_response);
    }
    sidebarProvider?.refresh(history);
    const rowsHtml = results.map((r, i) => {
        const scores = (r.scores || {});
        const reasons = (r.reasons || {});
        const response = r.agent_response || '';
      ...
      <div class="result-summary">
        ...
        <span class="result-query">${escapeHtml((r.query || '').length > 60 ? r.query.substring(0, 60) + '...' : r.query || '')}</span>
    ...
    const panel = vscode.window.createWebviewPanel('evalResults', `Results: ${fileName}`, vscode.ViewColumn.One, { enableScripts: true, retainContextWhenHidden: true });
    const now2 = new Date();
...
<body>
  <div class="header">
    ...
    <div class="meta">${results.length} quer${results.length === 1 ? 'y' : 'ies'} \u00b7 ${results.filter((r) => r.passed).length} passed \u00b7 ${results.filter((r) => !r.passed).length} failed</div>
  </div>
...
function addToHistory(query, scores, passed, id, response, expected) {
        ...
        webviewView.webview.onDidReceiveMessage(async (msg) => {
            switch (msg.command) {
                ...
                case 'openDashboard':
                    ...
                    break;
                case 'openDocs':
                    vscode.env.openExternal(vscode.Uri.parse('https://www.aievaluator.dev/tutorials'));
                    break;
                case 'setApiKey':
                    await vscode.commands.executeCommand('aievaluator.setAPIKey');
                    break;
                case 'initProject':
                    await vscode.commands.executeCommand('aievaluator.init');
                    break;
                case 'evaluateFile':
                    await vscode.commands.executeCommand('aievaluator.evaluateFile');
                    break;
                case 'addCustomEval':
                    await vscode.commands.executeCommand('aievaluator.addCustomEvaluator');
