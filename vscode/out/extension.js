...
function activate(context) {
    ...
    context.subscriptions.push(vscode.languages.registerCodeLensProvider({ language: 'json', pattern: '**/evals/**/*.json' }, new EvalCodeLensProvider()));
    context.subscriptions.push(vscode.languages.registerCodeLensProvider({ language: 'jsonc', pattern: '**/evals/**/*.json' }, new EvalCodeLensProvider()));
...
// ═══════════════════════════════════════════════════════════════════
async function pickEvalOptions(query) {
    // Step 1: pick agent
...
async function evaluateDataset(document) {
    ...
    await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: `Evaluating ${count} queries...` }, async () => {
        try {
            ...
            const passIcon = (data.results || []).every((r) => r.passed) ? '✅' : '❌';
            ...
            vscode.window.showInformationMessage(`AI Evaluator: ${(score * 100).toFixed(0)}% overall · ${count} rows ${passIcon}`);
        }
        catch (e) {
            const msg = e instanceof Error ? e.message : 'Unknown error';
            vscode.window.showErrorMessage(`AI Evaluator: ${msg}`);
